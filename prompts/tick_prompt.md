...
4. **Decide, constrained by the mode.**
   - **IMPLEMENTATION mode:** read `state/campaign_tu.md`. If empty / `functionally_complete` / `blocked` → pick ONE new TU, write it to `campaign_tu.md`, **begin work this same tick** (don't pick-and-exit). Otherwise → continue the existing campaign TU. **You do not decide when this tick ends** — work continuously until your session is exhausted. If you finish a function, start the next one. If you decode the next function into notes, immediately implement it; do NOT journal "ready next tick" and stop. Only mark the campaign `functionally_complete` (or `blocked`) when the TU genuinely is. Related/dependency TUs are allowed under the carve-out in `tick_focus.md`.
...
7. **Journal — incrementally, after each commit.** Append to `state/journal/<today>.md` *as you work*, not only at end-of-tick. Your session may end mid-function; you want progress captured in the journal before that happens. **Lead with the tick's mode and headline outcome so far** (e.g. "tick 12 [IMPLEMENTATION]: wrote SunGlass.cpp 0→100%, finished MapObjGate logic +3 fns"). One paragraph + bullets is fine. Be honest about failures. **Do not** mirror this content into `goals.md` — that file is strategy, not log. **And critically: do not use writing the journal as a signal to stop working.** Journal updates are background record-keeping, not an exit cue.
...
11. **Schedule next tick.** Write `state/next_tick.json` with a useful `reason` describing what the next tick should focus on (e.g. "continue MapObjSirena polish on perform"). The `wake_at` you write is **ignored by the daemon** — the operator controls the tick interval via the dashboard ("every 5/15/30/60 min..."), and the daemon rewrites `wake_at` to `now + interval` after every tick. Just write the time as "30 minutes from now" by convention; only the `reason` field matters. The *mode* of the next tick is also set by the daemon (auto-alternates) — don't try to override it.
