...
_FOCUS_BODY = {
    "implementation": (
        ...
        "## When this tick ends — DO NOT decide to stop\n\n"
        "**You do not choose when this tick ends.** The agent process (you)\n"
        "ends when your context fills or your session limits are hit; that's\n"
        "the only natural exit. There is **no '+0.2%' floor**, no\n"
        "'natural checkpoint' off-ramp, no 'next chunk needs fresh context'\n"
        "rationalization. Those were loopholes in earlier prompts — they're\n"
        "removed.\n\n"
        "Your job until session-end: **keep implementing functions on the\n"
        "campaign TU.** If you finish one, start the next one. If you decode\n"
        "a function into notes, immediately turn the notes into source and\n"
        "implement it — don't just journal 'ready to implement next tick'\n"
        "and stop. That's the failure mode this section exists to prevent.\n\n"
        "The campaign **status** is what changes, not the tick:\n\n"
        "- **TU functionally complete** → set `Status: functionally_complete`\n"
        "  in `state/campaign_tu.md`. The next IMPL tick picks a new TU.\n"
        "- **TU blocked** (genuine cross-TU dependency you cannot resolve\n"
        "  here — missing class hierarchy in another TU, unreconstructable\n"
        "  data table, etc.) → mark `Status: blocked` with the reason. The\n"
        "  next IMPL tick picks a different TU.\n"
        "- **Neither yet** → `Status: in_progress`. Keep working. The next\n"
        "  IMPL tick continues this same TU from wherever you left off.\n\n"
        ...
        "**Update journal/goals/notes/memory incrementally as you work**\n"
        "(after each commit, not at end-of-tick). That way if your session\n"
        "ends mid-function, your progress is captured.\n\n"
