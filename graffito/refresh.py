...
_FOCUS_BODY = {
    "implementation": (
        ...
        "## Stop criteria (when this tick may end)\n\n"
        "You may end this implementation tick only when **one** of these is true:\n\n"
        "1. **The fuzzy match % gained ≥ +0.2%** since the start of this tick\n"
        "   (compare `build/GMSJ01/report.json` now vs at tick start). OR\n"
        "2. **The campaign TU is functionally complete** — every function has\n"
        "   correct logic implemented (mark it complete in `state/campaign_tu.md`).\n\n"
        "If neither holds, **keep working on the same TU** — implement more\n"
        "functions, fix more logic, write more source. Don't end early because\n"
        "you're tired or because progress feels slow. The user's overriding\n"
        "concern is compute → percent efficiency; ending below the floor wastes\n"
        "the tick's startup overhead.\n\n"
        "**One narrow exception:** if you discover the campaign TU is genuinely\n"
        "blocked (missing class hierarchy in another TU, data table you cannot\n"
        "reconstruct), say so explicitly in the journal, mark the campaign\n"
        "`Status: blocked` with the reason, and you may end the tick — the\n"
        "next implementation tick will pick a different TU.\n\n"
